  ...
  return (
    ...
    >
            ...
            <div className="flex items-center gap-2 md:pr-16 pr-0">
                ...
                <img className="h-[3ch] bg-transparent" src="/logo-oneup.png" alt="logo" />
                </a>
            </div>
            ...
            <div className="flex flex-row justify-end"> 
              <div className="flex flex-row justify-center items-center">
                  ...
                  <div
                  className={`fixed lg:static top-[8.5ch] left-1/2 transform ${
              ...
              >
                  ...
                  <div className="flex-1 flex flex-col md:flex-row items-start justify-between gap-5 p-4 md:p-0">
                      {/* Navbar items */}
                      <ul className="group flex flex-col lg:flex-row pl-3 gap:6 lg:gap-8 mx-3 text-base text-zinc-50 lg:text-zinc-50 font-normal ">
                          {navItems.map((item) => (
                              <li ref={(el) => (menuRef.current[item.id] = el)}
                              key={item.id} 
                              onMouseEnter={() => setHoveredNavItem(item.id)}
                               onMouseLeave={() => setHoveredNavItem(null)} className="relative m-3 lg:m-0 origin-center border-l-2 border-transparent lg:hover:border-zinc-100 ">
                                  <a className="hoverTL py-[23px] px-2" href={item.path}>
                                      {item.name}
                                  </a>
                                  {item.hasDropdown && hoveredNavItem === item.id && (
                                    <div className='absolute z-[9999] hidden lg:group-hover:block w-[250px] bg-black -left-[16px] p-2 mt-6 text-zinc-400 shadow-md'>
                                    <ul>
              ...
              <div className="flex flex-row items-center gap-2">
                ...
                <button className="contact hidden sm:flex justify-center w-fit px-6 py-2 rounded-3xl text-thin text-zinc-50 bg-transparent cursor-pointer items-end gap-1 border-2 border-zinc-50 sm:hover:bg-red-800">
              ...
              <div className={`w-[380px] h-screen bg-black text-white pt-8 pl-9 flex flex-col fixed top-0 right-0 transition-transform duration-300 ease-in-out transform flex-1 ${isContactOpen ? 'translate-x-0 z-50' : 'translate-x-full' } `}>
      {/* Close Button */}
      ...
      <div className="mt-6">
        <img className="h-10 mb-6" src="/logo-oneup.png" alt="logo" />

        <p className="text-md font-normal text-gray-300">
          Creative Modern Agency and Portfolio WordPress Theme.
        </p>
      </div>
